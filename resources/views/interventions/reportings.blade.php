...
foreach ($usagers as $usager) {
	$id_quartier = $usager->quartier;
	$quartier = DB::table('quartiers')->where('id', $id_quartier)->first();
	$usager->quartier = $quartier->nom;

	$usager_array .= '"'. $usager->quartier.'",';

	//count $usagers by quartier
	$usagers_by_quartier = DB::table('usagers')->where('quartier', $id_quartier)->count();

	$usager_nb .= $usagers_by_quartier.",";
...
?>
<h2>Total Interventions ({{ $total_interventions }}) : </h2> : 
              ...
              <script>
                document.addEventListener("DOMContentLoaded", () => {
                  new Chart(document.querySelector('#pieChart'), {
                    ...
                    data: {
                      labels: [
                        'en cours  ({{ $total_interventions_en_cours }})',
                        'réglé ({{ $total_interventions_cloturees }})',
                       
                      ],
                      datasets: [{
                        label: 'My First Dataset',
                        data: [{{ $total_interventions_en_cours }},  {{ $total_interventions_cloturees }}],
                        backgroundColor: [
                          'rgb(255, 99, 132)',
                          'rgb(54, 162, 235)',
                          'rgb(255, 205, 86)'
...
<h2>Total usagers par quartier: </h2>
